(function () {
  ...
  function creerBanniere() {
    ...
    icone.className = "notification-banniere-icone";
    ...
    return { banniere, icone, message, fermer };
  }

  function afficherBanniere(notification) {
    const ui = creerBanniere();
    const { banniere, icone, message, fermer } = ui;
    let timer = null;
    ...
    icone.textContent = notification.niveau === "critical" ? "!" : notification.niveau === "warning" ? "!" : "i";
  ...
  async function initialiserNotifications() {
    ...
    const nonLues = actives.filter((n) => !dejaVue(n.id));
    if (!nonLues.length) {
      return;
    }

    for (const notification of nonLues) {
      await afficherBanniere(notification);
    }
  }
